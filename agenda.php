...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_single') {
        ...
        $title = $_POST['title'] ?? 'Horário Reservado';
        $timeStart = $_POST['start'];
        $timeEnd = $_POST['end'];
        
        $startDate = new DateTime($_POST['start_date'] ?? $selectedDateStr);
        $endDate = new DateTime($_POST['end_date'] ?? $selectedDateStr);
        $weekdays = $_POST['weekdays'] ?? []; // Empty means ONLY the specific date(s) naturally in range, or if single day
        
        // If it's a single day submission (old behavior or simple form), ensure loop works once
        if (empty($weekdays) && $startDate == $endDate) {
             // Just one iteration
        }
        ...
        while ($currentDate <= $endDate) {
            ...
            // 1. If weekdays ARE selected, current day must be in them.
            // 2. If weekdays are NOT selected (empty), we block every day in range (usual behavior for date ranges) OR just the single day.
            
            $dayNum = $currentDate->format('w');
            $shouldBlock = (empty($weekdays) || in_array($dayNum, $weekdays));

            if ($shouldBlock) {
                 $dateStr = $currentDate->format('Y-m-d');
                 $start_at = $dateStr . ' ' . $timeStart . ':00';
                 $end_at = $dateStr . ' ' . $timeEnd . ':00';
                 ...
                 $stmt->execute([$title, $start_at, $end_at]);
            }
        ...
        if (isset($_POST['divide_slots']) && $_POST['divide_slots'] === 'on') {
            $start = new DateTime($date . ' ' . $time_start);
            $end = new DateTime($date . ' ' . $time_end);
            ...
            while ($start < $end) {
                ...
                if ($slotEnd > $end) {
                    break; // Stop if the next hour exceeds the end time
                }
                
                $s_time = $slotStart->format('H:i');
                $e_time = $slotEnd->format('H:i');
                
                // Check if slot already exists
                $stmt = $pdo->prepare("SELECT id FROM available_slots WHERE date = ? AND time_start = ?");
                $stmt->execute([$date, $s_time]);
                
                if (!$stmt->fetch()) {
                    $stmtIns = $pdo->prepare("INSERT INTO available_slots (date, time_start, time_end, is_booked, appointment_id) VALUES (?, ?, ?, 0, NULL)");
                    $stmtIns->execute([$date, $s_time, $e_time]);
                }
    ...
    } elseif ($action === 'add_batch_slots') {
        $slots = json_decode($_POST['slots'], true); // Array of {start: 'HH:mm', end: 'HH:mm'}
        $startDate = new DateTime($_POST['start_date']);
        $endDate = new DateTime($_POST['end_date']);
        $weekdays = $_POST['weekdays'] ?? []; // Array of day numbers 0-6 (Sun-Sat) or empty for all
        
        $currentDate = clone $startDate;
        
        while ($currentDate <= $endDate) {
            // Check if current day of week is in selected weekdays (if any selection made)
            if (empty($weekdays) || in_array($currentDate->format('w'), $weekdays)) {
                $dateStr = $currentDate->format('Y-m-d');
                ...
                foreach ($slots as $slot) {
                    ...
                    // Check duplicate
                    $stmt = $pdo->prepare("SELECT id FROM available_slots WHERE date = ? AND time_start = ?");
                    $stmt->execute([$dateStr, $timeStart]);
                    
                    if (!$stmt->fetch()) {
                        $stmtIns = $pdo->prepare("INSERT INTO available_slots (date, time_start, time_end, is_booked, appointment_id) VALUES (?, ?, ?, 0, NULL)");
                        $stmtIns->execute([$dateStr, $timeStart, $timeEnd]);
                    }
                }
            }
            $currentDate->modify('+1 day');
...
<!-- Modal: Gerador de Slots Visual -->
<dialog id="modalOpenSlot" class="p-0 rounded-2xl shadow-2xl w-full max-w-2xl backdrop:bg-black/20 border-0">
    <div class="w-full bg-white rounded-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div class="px-6 py-5 flex justify-between items-center border-b border-emerald-100 bg-emerald-50 shrink-0">
            <h2 class="font-serif text-xl text-emerald-800 flex items-center gap-2">
                <i data-lucide="wand-2" class="w-5 h-5"></i> Gerador de Horários
            </h2>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                <div class="col-span-2 md:col-span-1">
                    <label class="block text-[10px] uppercase text-gray-500 mb-1.5 font-semibold tracking-wider">Início</label>
                    <input type="time" id="gen_start" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm" value="08:00">
                </div>
                <div class="col-span-2 md:col-span-1">
                    <label class="block text-[10px] uppercase text-gray-500 mb-1.5 font-semibold tracking-wider">Fim</label>
                    <input type="time" id="gen_end" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm" value="18:00">
                </div>
                <div>
                    <label class="block text-[10px] uppercase text-gray-500 mb-1.5 font-semibold tracking-wider">Duração (min)</label>
                    <input type="number" id="gen_duration" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm" value="50" step="5">
                </div>
                <div>
                    <label class="block text-[10px] uppercase text-gray-500 mb-1.5 font-semibold tracking-wider">Pausa (min)</label>
                    <input type="number" id="gen_gap" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm" value="10" step="5">
                </div>
                <div class="col-span-2 md:col-span-4">
                    <button type="button" onclick="generateVisualSlots()" class="w-full py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700 transition-colors shadow-sm flex items-center justify-center gap-2">
                        <i data-lucide="refresh-cw" class="w-4 h-4"></i> Gerar Blocos
                    </button>
                </div>
            </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-2">
                    <label class="block text-xs font-semibold text-gray-700 uppercase tracking-wider">Selecione os Horários</label>
                    <div class="flex gap-2">
                        <button type="button" onclick="selectAllSlots(true)" class="text-[10px] text-emerald-600 hover:underline">Todos</button>
                        <span class="text-gray-300">|</span>
                        <button type="button" onclick="selectAllSlots(false)" class="text-[10px] text-gray-400 hover:text-gray-600 hover:underline">Nenhum</button>
                    </div>
                </div>
                <div id="slots_container" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                    <!-- Slots gerados via JS -->
                    <div class="col-span-full py-8 text-center text-gray-400 text-sm border-2 border-dashed border-gray-100 rounded-xl">
                        Configure os horários acima e clique em "Gerar Blocos"
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-gray-100 pt-6">
                <label class="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-3">Aplicar Para</label>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-[10px] uppercase text-gray-500 mb-1.5">Data Inicial</label>
                        <input type="date" id="apply_start" value="<?php echo $selectedDateStr; ?>" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm">
                    </div>
                    <div>
                        <label class="block text-[10px] uppercase text-gray-500 mb-1.5">Data Final</label>
                        <input type="date" id="apply_end" value="<?php echo $selectedDateStr; ?>" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm">
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    foreach($daysMap as $idx => $name): 
                    ?>
                    <label class="flex items-center gap-2 p-2 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors bg-white">
                        <input type="checkbox" name="weekdays[]" value="<?php echo $idx; ?>" class="w-4 h-4 text-emerald-600 rounded border-gray-300 focus:ring-emerald-500" <?php echo ($selectedDate->format('w') == $idx) ? 'checked' : ''; ?>>
                        <span class="text-xs font-medium text-gray-700"><?php echo substr($name, 0, 3); ?></span>
                    </label>
                    <?php endforeach; ?>
                </div>
            </div>
        ...
        <div class="p-6 border-t border-gray-100 bg-gray-50 shrink-0">
            <button onclick="submitBatchSlots()" class="w-full py-3 bg-gray-900 text-white rounded-xl font-medium hover:bg-black transition-colors shadow-lg flex items-center justify-center gap-2">
                <i data-lucide="check-circle" class="w-5 h-5"></i> Confirmar Disponibilidade
            </button>
        </div>
    </div>
</dialog>
    ...
    <div class="w-full bg-white rounded-2xl overflow-hidden">
        ...
        <form method="POST" class="p-6 space-y-4">
            ...
            <div class="pt-4 border-t border-gray-100">
                <button type="button" onclick="document.getElementById('blockRecurrence').classList.toggle('hidden')" class="text-xs font-semibold text-gray-700 flex items-center gap-2 mb-3 hover:text-gray-900 transition-colors">
                    <i data-lucide="repeat" class="w-3.5 h-3.5"></i> Configurar Repetição (Opcional)
                </button>
                
                <div id="blockRecurrence" class="hidden space-y-4 bg-gray-50 p-3 rounded-xl border border-gray-100">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-[10px] uppercase text-gray-500 mb-1.5">De</label>
                            <input type="date" name="start_date" value="<?php echo $selectedDateStr; ?>" class="w-full px-2 py-2 bg-white border border-gray-200 rounded-lg text-xs">
                        </div>
                        <div>
                            <label class="block text-[10px] uppercase text-gray-500 mb-1.5">Até</label>
                            <input type="date" name="end_date" value="<?php echo $selectedDateStr; ?>" class="w-full px-2 py-2 bg-white border border-gray-200 rounded-lg text-xs">
                        </div>
                    </div>
                    
                    <div>
                        <label class="block text-[10px] uppercase text-gray-500 mb-1.5">Dias da Semana</label>
                        <div class="flex flex-wrap gap-2">
                           <?php 
                            $daysMap = ['D', 'S', 'T', 'Q', 'Q', 'S', 'S'];
                            foreach($daysMap as $idx => $name): 
                            ?>
                            <label class="flex items-center gap-1.5 p-1.5 border border-gray-200 rounded bg-white hover:bg-gray-100 cursor-pointer">
                                <input type="checkbox" name="weekdays[]" value="<?php echo $idx; ?>" class="w-3.5 h-3.5 text-gray-800 rounded border-gray-300">
                                <span class="text-[10px] font-medium text-gray-600"><?php echo $name; ?></span>
                            </label>
                            <?php endforeach; ?>
                        </div>
                    </div>
            ...
            <button type="submit" class="w-full py-3 bg-gray-800 text-white rounded-lg font-medium hover:bg-gray-700 transition-colors mt-2 flex items-center justify-center gap-2">
                <i data-lucide="lock" class="w-4 h-4"></i> Bloquear Agenda
            </button>
        </form>
    </div>
</dialog>
...
<script>
    ...
    // --- VISUAL SLOT GENERATOR LOGIC ---
    let generatedSlots = []; // Stores {start: 'HH:mm', end: 'HH:mm', selected: true/false}

    function generateVisualSlots() {
        const startStr = document.getElementById('gen_start').value;
        const endStr = document.getElementById('gen_end').value;
        const duration = parseInt(document.getElementById('gen_duration').value) || 60;
        const gap = parseInt(document.getElementById('gen_gap').value) || 0;
        ...
        [h, m] = endStr.split(':').map(Number);
        let endMin = h * 60 + m;
        
        while (currentMin + duration <= endMin) {
            // Format Start
            let startH = Math.floor(currentMin / 60);
            let startM = currentMin % 60;
            let startFmt = `${String(startH).padStart(2, '0')}:${String(startM).padStart(2, '0')}`;
            
            // Calc End
            let slotEndMin = currentMin + duration;
            let endH = Math.floor(slotEndMin / 60);
            let endM = slotEndMin % 60;
            let endFmt = `${String(endH).padStart(2, '0')}:${String(endM).padStart(2, '0')}`;
            
            // Store
            generatedSlots.push({
                id: generatedSlots.length,
                start: startFmt,
                end: endFmt,
                selected: true
            });
    ...
    function renderSlots() {
        ...
        if (generatedSlots.length === 0) {
            container.innerHTML = '<div class="col-span-full py-8 text-center text-gray-400 text-sm border-2 border-dashed border-gray-100 rounded-xl">Nenhum horário cabe neste intervalo.</div>';
            return;
        }

        // Get date for display
        const dateInput = document.getElementById('apply_start').value;
        let dateDisplay = '';
        if (dateInput) {
        ...
        generatedSlots.forEach((slot, index) => {
            ...
            btn.type = 'button';
            btn.className = `p-2 rounded-lg text-xs font-medium border transition-all flex flex-col items-center justify-center gap-0.5 opacity-0 transform translate-y-2 ${
                slot.selected 
                ? 'bg-emerald-50 border-emerald-400 text-emerald-700 shadow-sm' 
                : 'bg-white border-gray-200 text-gray-400 hover:bg-gray-50'
            }`;
            btn.onclick = () => toggleSlot(slot.id);
            ...
            if (dateDisplay) {
                ...
                dateSpan.className = `text-[9px] ${slot.selected ? 'text-emerald-600/70' : 'text-gray-300'}`;
            ...
            icon.dataset.lucide = slot.selected ? 'check-circle-2' : 'circle';
            icon.className = `w-3 h-3 mt-1 ${slot.selected ? 'text-emerald-500' : 'text-gray-300'}`;
    ...
    function selectAllSlots(selected) {
        generatedSlots.forEach(s => s.selected = selected);
        renderSlots();
    }

    function submitBatchSlots() {
        const selected = generatedSlots.filter(s => s.selected);
        ...
        form.method = 'POST';
        form.innerHTML = `
            <input type="hidden" name="action" value="add_batch_slots">
            <input type="hidden" name="slots" value='${JSON.stringify(selected)}'>
            <input type="hidden" name="start_date" value="${document.getElementById('apply_start').value}">
            <input type="hidden" name="end_date" value="${document.getElementById('apply_end').value}">
            ${weekdays.map(d => `<input type="hidden" name="weekdays[]" value="${d}">`).join('')}
        `;
